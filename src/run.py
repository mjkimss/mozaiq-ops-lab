"""Run the hub model for one scenario and print GA-vs-optimal.   python -m src.run --scenario current|expansion
...
def main():
    ...
    cfg = load_config()
    v2 = cfg["v2"]
    inst = build_instance(scenario, cfg)
    villas, hubs, stats = inst["villas"], inst["hubs"], inst["travel_stats"]
    n_pairs = sum(stats.values())

    print(f"== scenario: {scenario}   (SIMULATED villas, PLACEHOLDER costs: not MOZAIQ data) ==")
    ...
    print(f"travel data: {n_pairs} villa-hub pairs: {stats['cache']} from cache, {stats['osrm']} fresh OSRM, "
    ...
    print(f"  cost breakdown (GA decoder on this hub set): " + ", ".join(f"{k} {krw(float(parts[k][0]))}" for k in ("transport", "drive", "vendor", "fixed", "capacity")))
    ...
    for method in SELECTION:
        for seed in seeds:
            ...
            rows.append({"seed": seed, "method": method, "cost": r["cost"], "gap_pct": 100 * (r["cost"] / ex["cost"] - 1),
    ...
    with open(ROOT / "outputs" / f"v2_{scenario}_runs.csv", "w", newline="") as f:
